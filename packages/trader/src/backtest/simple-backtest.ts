...
async function runBacktest(symbol: string, data: DataFrame) {
  ...
  if (trades.length > 0) {
    ...
    trades.slice(0, 5).forEach((trade, i) => {
      console.log(`  ${i + 1}. ${trade.direction.toUpperCase()} | Entry: $${trade.entryPrice.toFixed(2)} | Exit: $${trade.exitPrice.toFixed(2)} | P&L: $${trade.profit.toFixed(2)} | Reason: ${trade.exitReason || 'unknown'}`);
    });
  }
