/**
 * Deep Analysis of KELTNER_MR Strategy
 *
...
import { createKeltnerMR } from '../strategies/mr/keltner-mr.strategy.js';
import {
  type BacktestConfig,
  type Trade,
  type Direction,
...
// ============================================================================
// CONFIGURATION - Optimal params from grid search
// ============================================================================

const CONFIG = {
  // KELTNER_MR optimized params from grid search
  strategy: {
    kcEmaPeriod: 20,
    kcAtrPeriod: 14,
    kcMultiplier: 1.5,
  ...
  backtest: {
    initialBalance: 1000,
    stakePct: 0.04, // 4%
    multiplier: parseInt(process.env.MULTIPLIER || '300', 10), // Default 300×
    maxBarsInTrade: 15,
  },
};
...
async function main() {
  ...
  console.log(`🔍 DEEP ANALYSIS: KELTNER_MR ${CONFIG.backtest.multiplier}× Strategy`);
  console.log('═'.repeat(70));
  ...
  console.log(`   Strategy params: KC${CONFIG.strategy.kcMultiplier} RSI${CONFIG.strategy.rsiOversold}/${CONFIG.strategy.rsiOverbought} SL${CONFIG.strategy.slMultiplier} ADX${CONFIG.strategy.adxThreshold}`);
  console.log(`   Backtest params: ${CONFIG.backtest.multiplier}× stake ${CONFIG.backtest.stakePct * 100}%`);
  ...
  if (process.env.RUN_MC === 'true' || process.env.RUN_ALL === 'true') {
    console.log('\n🎲 MONTE CARLO SIMULATION');
    console.log('─'.repeat(50));
    monteCarlo = runMonteCarlo(trades, 1000);
    printMonteCarlo(monteCarlo);
  }

  // Out-of-sample validation
  let outOfSample: OOSResult | undefined;
  if (process.env.RUN_OOS === 'true' || process.env.RUN_ALL === 'true') {
    console.log('\n🔬 OUT-OF-SAMPLE VALIDATION');
    console.log('─'.repeat(50));
    outOfSample = runOutOfSampleValidation(candles, tradingDays);
    printOOS(outOfSample);
  }

  // Generate recommendations
  const recommendations = generateRecommendations(basicMetrics, streakAnalysis, monthlyAnalysis, sessionAnalysis, riskMetrics, monteCarlo, outOfSample);
  console.log('\n💡 RECOMMENDATIONS');
  console.log('─'.repeat(50));
  ...
  const outputPath = resolve(process.cwd(), `analysis-output/keltner_mr_deep_analysis_${CONFIG.backtest.multiplier}x.json`);
...
function runDetailedBacktest(candles: Candle[]): DetailedTrade[] {
  const strategy = createKeltnerMR({
    kcEmaPeriod: CONFIG.strategy.kcEmaPeriod,
    kcAtrPeriod: CONFIG.strategy.kcAtrPeriod,
    kcMultiplier: CONFIG.strategy.kcMultiplier,
    rsiOversold: CONFIG.strategy.rsiOversold,
    rsiOverbought: CONFIG.strategy.rsiOverbought,
    slMultiplier: CONFIG.strategy.slMultiplier,
    maxBars: CONFIG.strategy.maxBars,
  ...
  const btConfig: BacktestConfig = {
    ...
    takeProfitPct: 0.005, // Will be overridden by strategy
    stopLossPct: 0.005,
...
function runMonteCarlo(trades: DetailedTrade[], iterations: number): MonteCarloResult {
  const results: number[] = [];
  const maxDrawdowns: number[] = [];
  const numTrades = trades.length;

  // Extract just the P&L values for faster sampling
  const pnlValues = trades.map(t => t.pnl);

  for (let i = 0; i < iterations; i++) {
    // Bootstrap sampling: randomly sample WITH REPLACEMENT
    // This creates different possible "years" of trading
    let equity = CONFIG.backtest.initialBalance;
    let peak = equity;
    let maxDD = 0;
    ...
    for (let j = 0; j < numTrades; j++) {
      ...
      const randomIdx = Math.floor(Math.random() * numTrades);
      const pnl = pnlValues[randomIdx]!;
    ...
    results.push(equity - CONFIG.backtest.initialBalance);
  ...
  const probabilityOfProfit = results.filter(r => r > 0).length / iterations * 100;
  ...
  return {
    ...
    probabilityOfProfit,
    avgMaxDrawdown: maxDrawdowns.reduce((a, b) => a + b, 0) / iterations,
  };
}
...
function runOutOfSampleValidation(candles: Candle[], tradingDays: number): OOSResult {
  // Split 70/30
  ...
  const inSampleDays = Math.floor(tradingDays * 0.7);
  const outOfSampleDays = tradingDays - inSampleDays;

  console.log(`   In-sample: ${inSampleCandles.length} candles (${inSampleDays} days)`);
  console.log(`   Out-of-sample: ${outOfSampleCandles.length} candles (${outOfSampleDays} days)`);

  // Run backtest on in-sample
  const inSampleTrades = runDetailedBacktestOnCandles(inSampleCandles);
  const inSampleMetrics = calculateBasicMetrics(inSampleTrades, inSampleDays);

  // Run backtest on out-of-sample
  const outOfSampleTrades = runDetailedBacktestOnCandles(outOfSampleCandles);
  const outOfSampleMetrics = calculateBasicMetrics(outOfSampleTrades, outOfSampleDays);

  // Check consistency (OOS should be at least 50% of IS performance)
  const pfRatio = outOfSampleMetrics.profitFactor / inSampleMetrics.profitFactor;
  const wrRatio = outOfSampleMetrics.winRate / inSampleMetrics.winRate;
  const isConsistent = pfRatio >= 0.5 && wrRatio >= 0.8;

  return {
    inSamplePnL: inSampleMetrics.netPnL,
    outOfSamplePnL: outOfSampleMetrics.netPnL,
    inSamplePF: inSampleMetrics.profitFactor,
    outOfSamplePF: outOfSampleMetrics.profitFactor,
    inSampleWR: inSampleMetrics.winRate,
    outOfSampleWR: outOfSampleMetrics.winRate,
    isConsistent,
  };
}

function runDetailedBacktestOnCandles(candles: Candle[]): DetailedTrade[] {
  // This is a simplified version that runs on a subset of candles
  const strategy = createKeltnerMR({
    kcEmaPeriod: CONFIG.strategy.kcEmaPeriod,
    kcAtrPeriod: CONFIG.strategy.kcAtrPeriod,
    kcMultiplier: CONFIG.strategy.kcMultiplier,
...
function generateRecommendations(
  ...
  if (basicMetrics.winRate >= 35) {
    recs.push(`✅ Win rate de ${basicMetrics.winRate.toFixed(1)}% es bueno para MR.`);
  } else {
    recs.push(`⚠️ Win rate de ${basicMetrics.winRate.toFixed(1)}% es moderado.`);
  }

  // Win/Loss ratio
  if (basicMetrics.winLossRatio >= 1.5) {
    recs.push(`✅ Win/Loss ratio de ${basicMetrics.winLossRatio.toFixed(2)} es sólido.`);
  }

  // Streak analysis
  if (streakAnalysis.maxLoseStreak <= 10) {
    recs.push(`✅ Max losing streak de ${streakAnalysis.maxLoseStreak} es manejable.`);
  } else if (streakAnalysis.maxLoseStreak <= 15) {
    recs.push(`⚠️ Max losing streak de ${streakAnalysis.maxLoseStreak}. Usar stake conservador.`);
  } else {
    recs.push(`🔴 Max losing streak de ${streakAnalysis.maxLoseStreak} es alto.`);
  }

  // Monthly consistency
  const posMonths = monthlyAnalysis.filter((m) => m.pnl > 0).length;
  const totalMonths = monthlyAnalysis.length;
  if (posMonths >= totalMonths * 0.75) {
    recs.push(`✅ ${posMonths}/${totalMonths} meses positivos. Excelente consistencia.`);
  } else if (posMonths >= totalMonths * 0.5) {
    recs.push(`⚠️ ${posMonths}/${totalMonths} meses positivos. Consistencia moderada.`);
  } else {
    recs.push(`🔴 Solo ${posMonths}/${totalMonths} meses positivos. Baja consistencia.`);
  }

  // Drawdown
  if (riskMetrics.maxDrawdownPct <= 15) {
    recs.push(`✅ Max drawdown ${riskMetrics.maxDrawdownPct.toFixed(1)}% es bajo.`);
  } else if (riskMetrics.maxDrawdownPct <= 25) {
    recs.push(`⚠️ Max drawdown ${riskMetrics.maxDrawdownPct.toFixed(1)}% es moderado.`);
  } else {
    recs.push(`🔴 Max drawdown ${riskMetrics.maxDrawdownPct.toFixed(1)}% es alto.`);
  }

  // Sharpe Ratio
  if (riskMetrics.sharpeRatio >= 2) {
    recs.push(`✅ Sharpe Ratio ${riskMetrics.sharpeRatio.toFixed(2)} es excelente.`);
  } else if (riskMetrics.sharpeRatio >= 1) {
    recs.push(`✅ Sharpe Ratio ${riskMetrics.sharpeRatio.toFixed(2)} es bueno.`);
  } else {
    recs.push(`⚠️ Sharpe Ratio ${riskMetrics.sharpeRatio.toFixed(2)} < 1.`);
  }

  // Monte Carlo
  if (monteCarlo) {
    if (monteCarlo.probabilityOfProfit >= 95) {
      recs.push(`✅ Monte Carlo: ${monteCarlo.probabilityOfProfit.toFixed(1)}% prob. de ganancia.`);
    } else if (monteCarlo.probabilityOfProfit >= 80) {
      recs.push(`⚠️ Monte Carlo: ${monteCarlo.probabilityOfProfit.toFixed(1)}% prob. de ganancia.`);
    } else {
      recs.push(`🔴 Monte Carlo: Solo ${monteCarlo.probabilityOfProfit.toFixed(1)}% prob. de ganancia.`);
    }
  }

  // Out of Sample
  if (outOfSample) {
    if (outOfSample.isConsistent) {
      recs.push(`✅ OOS validación pasada. PF ratio: ${(outOfSample.outOfSamplePF / outOfSample.inSamplePF).toFixed(2)}`);
    } else {
      recs.push(`🔴 OOS validación FALLIDA. Posible overfitting.`);
    }
  }
  ...
  if (bestSession && bestSession.pnl > 0) {
    recs.push(`💡 Mejor sesión: ${bestSession.session} (PF: ${bestSession.profitFactor.toFixed(2)})`);
  }

  // Final recommendation
  const passCount = recs.filter(r => r.startsWith('✅')).length;
  const failCount = recs.filter(r => r.startsWith('🔴')).length;

  if (failCount === 0 && passCount >= 5) {
    recs.push(`🟢 APTO para forward testing con stake 2-3%.`);
  } else if (failCount <= 1) {
    recs.push(`🟡 Forward test con precaución. Usar stake 1-2%.`);
  } else {
    recs.push(`🔴 Requiere más optimización antes de forward test.`);
  }
...
function printBasicMetrics(m: ReturnType<typeof calculateBasicMetrics>) {
  ...
  console.log(`  ROI:               ${((m.netPnL / CONFIG.backtest.initialBalance) * 100).toFixed(1)}%`);
...
function printRiskMetrics(r: RiskMetrics) {
  ...
  console.log(`  Risk of Ruin:         ${r.riskOfRuin.toFixed(4)}%`);
  console.log(`  Max Consecutive Loss: ${r.maxConsecutiveLosses}`);
  console.log(`  Avg Recovery Days:    ${r.avgRecoveryDays.toFixed(1)}`);
}

function printMonteCarlo(mc: MonteCarloResult) {
  console.log(`  P&L Distribution (1000 simulations):`);
  console.log(`    5th percentile:   $${mc.percentile5.toFixed(2)}`);
  console.log(`    25th percentile:  $${mc.percentile25.toFixed(2)}`);
  console.log(`    Median:           $${mc.median.toFixed(2)}`);
  console.log(`    75th percentile:  $${mc.percentile75.toFixed(2)}`);
  console.log(`    95th percentile:  $${mc.percentile95.toFixed(2)}`);
  console.log(`  Probability of Profit: ${mc.probabilityOfProfit.toFixed(1)}%`);
  console.log(`  Avg Max Drawdown:      ${mc.avgMaxDrawdown.toFixed(1)}%`);
}

function printOOS(oos: OOSResult) {
  console.log(`  In-Sample (70%):`);
  console.log(`    P&L:           $${oos.inSamplePnL.toFixed(2)}`);
  console.log(`    Profit Factor: ${oos.inSamplePF.toFixed(2)}`);
  console.log(`    Win Rate:      ${oos.inSampleWR.toFixed(1)}%`);
  console.log(`  Out-of-Sample (30%):`);
  console.log(`    P&L:           $${oos.outOfSamplePnL.toFixed(2)}`);
  console.log(`    Profit Factor: ${oos.outOfSamplePF.toFixed(2)}`);
  console.log(`    Win Rate:      ${oos.outOfSampleWR.toFixed(1)}%`);
  console.log(`  Consistency Check: ${oos.isConsistent ? '✅ PASSED' : '🔴 FAILED'}`);
}
