"""
Reversal Hunter Strategy - OPTIMIZED VERSION
Based on 30-day backtest results, optimized for better frequency and win rate
"""
...
class ReversalHunterBalancedStrategy(BaseBinaryStrategy):
    """
    OPTIMIZED Reversal Hunter Strategy
    - Optimized for 5-10 trades/day (reduced from 31/day)
    - Target win rate: 55%+ (improved from 50.16%)
    - Functional Anti-Martingale system
    - Only strongest patterns: Pin Bar + Engulfing (strength 3+)
    - Stricter S/R proximity requirement (50/100 minimum)
    - Mandatory RSI filter
    """

    params = (
        # OPTIMIZED - Pattern detection parameters
        ...
        ('wick_multiplier', 2.0),  # Wick > 2x body
        ('engulfing_ratio', 1.5),  # Engulfing > 1.5x body

        # OPTIMIZED - Indicator parameters
        ('ema_period', 20),  # EMA 20 for trend filter
        ('atr_period', 14),  # ATR 14
        ('rsi_period', 14),  # RSI 14 NOW MANDATORY
        ('rsi_oversold', 30),  # RSI < 30 for CALL
        ('rsi_overbought', 70),  # RSI > 70 for PUT

        # OPTIMIZED - Trading parameters (MODERATE)
        ('cooldown_seconds', 600),  # 10 min cooldown (was 300) - MODERATE
        ('max_concurrent_trades', 3),  # Max 3 concurrent
        ('min_strength_trade', 2),  # Min strength 2 (keep all patterns)
        ('min_sr_proximity', 40),  # Minimum S/R proximity 40/100 (was 30) - MODERATE
        ('rsi_required', False),  # Keep RSI as bonus, not mandatory - MODERATE
    ...
    def __init__(self):
        ...
        # Initialize Support/Resistance system
        self.sr_system = SupportResistance(
        ...
        print(f"🎯 OPTIMIZED Reversal Hunter Strategy initialized")
        print(f"   Min Confidence: {self.params.min_confidence}")
        print(f"   Min S/R Proximity: {self.params.min_sr_proximity}/100 (MODERATE +10 from 30)")
        print(f"   ATR Multiplier: {self.params.atr_multiplier}")
        print(f"   Wick Multiplier: {self.params.wick_multiplier}")
        print(f"   Min Strength Trade: {self.params.min_strength_trade} (All patterns)")
        print(f"   Cooldown: {self.params.cooldown_seconds}s (MODERATE +300s from 300s)")
        print(f"   Max Concurrent: {self.params.max_concurrent_trades}")
        print(f"   RSI Required: {self.params.rsi_required} (Bonus, not mandatory)")
    ...
    def generate_signal(self) -> Optional[str]:
        ...
        # Update Support/Resistance system
        self.sr_system.update(self.data, len(self.data))
        ...
        # OPTIMIZED: Check S/R proximity with stricter threshold
        sr_proximity = pattern['metadata'].get('sr_proximity', 0)
        min_sr_proximity = self.params.min_sr_proximity  # Now 50/100 (was 30)
        ...
        # OPTIMIZED: RSI is now MANDATORY if rsi_required is True
        rsi_bonus = rsi_filter['passed']
        if self.params.rsi_required and not rsi_bonus:
            return None  # RSI filter mandatory, skip trade
    ...
    def _execute_binary_trade(self, direction: str):
        ...
        # Track trade for Anti-Martingale
        trade_info = {
